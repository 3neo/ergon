...
public class CvController {
    ...
    @Autowired
    public CvController(CvService cvService, ReportService reportService) {
    ...
    /**
     * Generates a report for the cv.
     */
    @Operation(summary = "Generate cv report", description = "Generate a PDF report of cv by its id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Report generated successfully"),
            @ApiResponse(responseCode = "500", description = "Error generating report")
    })
    @GetMapping("/report/{id}")
    public void generateDemandsReport(HttpServletResponse response, @PathVariable int id) throws IOException {

        // Generate the PDF report as a byte array

        // Set response headers
        response.setContentType("application/pdf");
        response.setHeader("Content-Disposition", String.format("attachment; filename=\"cvCanada.pdf\"")); // Display in browser

        // Write the PDF bytes to the response output stream
        OutputStream outputStream = response.getOutputStream();
        reportService.generateReport(outputStream, id);
